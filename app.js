...
(function(){
  ...
  var loaderMinDuration = 3000; // 3 секунды минимум при старте
  ...
  function showLoader(text){
    ...
    // фиксируем старт только для первой (старта сцены) фазы
    if(!loaderStartupPhase && (text === 'Loading Cubiks...' || text === undefined)){
      loaderStartupPhase = true;
      loaderStartTime = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
    }
...
setupColorPipeline(renderer);
   ...
   // Studio floor + curved cove backdrop (no hard corner)
var plateSize = 26 * 1.4; // ~40% больше рабочего пространства

var floorColor = '#F1F3F6'; // базовый сероватый тон пола
var wallColor  = '#8B95A7'; // цвет стен / окружения

// Радиальный градиент для пола: плавный и заметный переход
function makeFloorRadialTexture(innerHex, midHex, outerHex){
  var size = 1024;
  ...
  // ЕСЛИ у тебя раньше были свои rInner / rOuter – поставь их сюда.
  var rInner = 0;            // центр
  var rOuter = size * 0.5;   // край круга

  var g = ctx.createRadialGradient(cx, cy, rInner, cx, cy, rOuter);

  // Делаем цвета контрастнее, чтобы градиент был виден
  var cInner = innerHex || '#FFFFFF';  // центр почти чисто белый
  var cMid   = midHex   || '#E5E7EB';  // средний мягкий серый
  var cOuter = outerHex || '#B0BACB';  // по краю заметно темнее

  // Плавное распределение: центр широкий, затем длинный переход к краю
  g.addColorStop(0.0, cInner);
  g.addColorStop(0.25, cInner); // держим светлый центр
  g.addColorStop(0.6, cMid);    // основная зона перехода
  g.addColorStop(1.0, cOuter);  // тёмный край
  ...
  // поддержка разных версий three.js
...
// создаём текстуру пола с явным, но плавным градиентом
var floorTexture = makeFloorRadialTexture(
  '#dcdcdcff',  // центр
  '#E5E7EB',  // середина
  '#b0b3b3ff'   // край
);

// --- Floor: круг, чтобы не было жёстких углов по краям ---
var floorMat = new THREE.MeshStandardMaterial({
  color: toLinear('#FFFFFF'), // не красит дополнительно, даёт текстуре честно работать
  map: floorTexture,
  roughness: 0.95,
  metalness: 0.0
});
...
var floor = new THREE.Mesh(
  new THREE.CircleGeometry(plateSize * 0.9, 72),
  floorMat
);
...
// --- Curved cove backdrop removed ---
// Раньше здесь создавалась большая сферическая "студия" вокруг сцены,
// но теперь фон рисуется полноэкранным градиентом в bgScene, поэтому
// геометрический cove убран, чтобы не перебивать мягкий градиент.

// Сильнее градиент: ближе начинаем туман и быстрее уходим в серый
// scene.fog disabled to keep gradient static relative to viewport
// scene.fog = null;
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    try { showLoader('Loading Cubiks...'); } catch(_){}
    embeddedInit();
  });
})();
